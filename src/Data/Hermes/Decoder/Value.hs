...
withObjectIter :: (ObjectIter -> Decoder a) -> Decoder a
withObjectIter f =
  withRunInIO $ \run ->
...
iterateOverFieldsMap
  ...
  -> ObjectIter
  -> Decoder (Map a b)
iterateOverFieldsMap fk fv iterPtr = do
  valPtr <- asks hValue
  withRunInIO $ \run ->
    F.alloca $ \lenPtr ->
      F.alloca $ \keyPtr ->
        run $ go M.empty keyPtr lenPtr valPtr
  where
...
iterateOverFields
  ...
  -> ObjectIter
  -> Decoder [(a, b)]
iterateOverFields fk fv iterPtr = do
  valPtr <- asks hValue
  withRunInIO $ \run ->
    F.alloca $ \lenPtr ->
      F.alloca $ \keyPtr ->
...
withUnorderedField f objPtr key = do
  vPtr <- asks hValue
  withRunInIO $ \run ->
...
withUnorderedOptionalField f objPtr key = do
  vPtr <- asks hValue
  withRunInIO $ \run ->
    Unsafe.unsafeUseAsCStringLen (T.encodeUtf8 key) $ \(cstr, len) ->
      run $ withPath key $ do
...
withCStringLen :: Text -> (F.CStringLen -> Decoder a) -> Decoder a
withCStringLen lbl f = do
  valPtr <- asks hValue
  withRunInIO $ \run ->
    F.alloca $ \strPtr ->
...
getRawByteString :: Decoder BS.ByteString
getRawByteString = do
  valPtr <- asks hValue
  withRunInIO $ \run ->
    F.alloca $ \strPtr ->
...
withArrayLen :: ((Array, Int) -> Decoder a) -> Decoder a
withArrayLen f = do
  val <- asks hValue
  withRunInIO $ \run ->
    allocaArray $ \arrPtr ->
...
withArrayIter :: (ArrayIter -> Decoder a) -> Decoder a
withArrayIter f = do
  valPtr <- asks hValue
  withRunInIO $ \run ->
...
iterateOverArray :: Decoder a -> ArrayIter -> Decoder [a]
iterateOverArray f iterPtr = do
  valPtr <- asks hValue
  go (0 :: Int) DList.empty valPtr
  where
    go !n !acc !valPtr = do
...
withArrayLenIter :: (ArrayIter -> Int -> Decoder a) -> Decoder a
withArrayLenIter f = do
  valPtr <- asks hValue
  withRunInIO $ \run ->
    allocaArrayIter $ \iterPtr -> do
      F.alloca $ \outLen -> do
...
iterateOverArrayLen :: G.Vector v a => Decoder a -> ArrayIter -> Int -> Decoder (v a)
iterateOverArrayLen f iterPtr len = do
  valPtr <- asks hValue
  withRunInIO $ \run -> do
    v <- GM.new len
    ...
    go !n !acc !valPtr = do
      ...
      if not isOver
        then do
          ...
          result <- DecoderPrim $ withPathIndex n $ f
