...
iterateOverArray f iterPtr = Decoder $ \valPtr ->
  withRunInIO $ \run ->
    run $ go (0 :: Int) DList.empty valPtr
  where
    go !n !acc !valPtr = do
  ...
  withRunInIO $ \run ->
    allocaArrayIter $ \iterPtr ->
      F.alloca $ \outLen -> do
        ...
        run $ do
          handleErrorCode "" err
          runDecoder (f iterPtr len) valPtr
...
iterateOverArrayLen f iterPtr len = Decoder $ \valPtr ->
  withRunInIO $ \run -> do
    v <- GM.new len
...
iterateOverFieldsMap
  ...
  -> ObjectIter
  -> Decoder (Map a b)
iterateOverFieldsMap fk fv iterPtr = Decoder $ \valPtr ->
  withRunInIO $ \run ->
    F.alloca $ \lenPtr ->
      F.alloca $ \keyPtr ->
...
iterateOverFields
  ...
  -> ObjectIter
  -> Decoder [(a, b)]
iterateOverFields fk fv iterPtr = Decoder $ \valPtr ->
  withRunInIO $ \run ->
    F.alloca $ \lenPtr ->
      F.alloca $ \keyPtr ->
